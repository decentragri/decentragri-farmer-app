...
func slot_data(data: Dictionary) -> void:
	...
	if data.has("interpretation") and typeof(data["interpretation"]) == TYPE_DICTIONARY and data["interpretation"].has("evaluation"):
		%OverAllLabel.text = data["interpretation"]["evaluation"]
	else:
		%OverAllLabel.text = "No evaluation"
